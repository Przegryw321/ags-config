...
/**
 * Get the time since last modified in microseconds.
 * Returns null if it can't get the modified date.
 * @param path - path to file
*/
export const file_age = (path: string): GLib.TimeSpan | null => {
  const file     = Gio.File.new_for_path(path);

  if (!file.query_exists(null)) return null;

  const fileInfo = file.query_info('time', Gio.FileQueryInfoFlags.NONE, null);
  const modified = fileInfo.get_modification_date_time();
  const now      = GLib.DateTime.new_now_utc();

  if (modified) {
    return now.difference(modified);
  }
